...
class FlutterOverlayWindow {
  ...
  /// Opens the device launcher (home screen) from the overlay isolate.
  ///
  /// How to use: call when the user taps “Leave” on a full-screen block dialog
  /// so the blocked app is no longer in the foreground.
  static Future<bool> goHome() async {
    try {
      return await _overlayChannel.invokeMethod<bool>('goHome') ?? false;
    } on PlatformException catch (error) {
      log("Error goHome: $error");
      return false;
    }
  }
