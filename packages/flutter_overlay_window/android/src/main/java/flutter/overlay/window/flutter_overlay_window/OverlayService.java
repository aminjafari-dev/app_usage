...
public class OverlayService extends Service implements View.OnTouchListener {
    ...
    /** Inset from every screen edge. */
    private static final int EDGE_PADDING_PX = 5;
    ...
    /** Sends the user to the launcher so a blocked app is no longer foreground. */
    private void goHome(MethodChannel.Result result) {
        try {
            ...
            Log.e("OverlayService", "Failed to go home", e);
    ...
    public static boolean moveOverlay(int x, int y) {
        if (instance != null && instance.flutterView != null) {
            if (instance.windowManager != null) {
                ...
                instance.clampToVisibleDisplay(params);
    ...
    private void clampToVisibleDisplay(WindowManager.LayoutParams params) {
        if (windowManager == null || flutterView == null || params.width <= 0 || params.height <= 0) {
            return;
        }
        int w = Math.max(params.width, flutterView.getWidth());
        int h = Math.max(params.height, flutterView.getHeight());
        Point screen = new Point();
        windowManager.getDefaultDisplay().getRealSize(screen);
        int pad = EDGE_PADDING_PX;
        int maxX = Math.max(0, (screen.x - w) / 2 - pad);
        int maxY = Math.max(pad, screen.y - h - pad);
        if (params.x < -maxX) params.x = -maxX;
        else if (params.x > maxX) params.x = maxX;
        if (params.y < pad + 20) params.y = pad + 150;
